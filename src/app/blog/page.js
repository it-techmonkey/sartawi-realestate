            ...
            {blogs.map((blog, i) => (
              <ScrollReveal key={blog.slug} delay={i * 0.05}>
                <li>
                  <Link
                    href={`/blog/${blog.slug}`}
                    className="block rounded-2xl border border-zinc-800 bg-zinc-900/40 p-6 sm:p-8 hover:border-[#e0b973]/50 hover:bg-zinc-900/60 transition-all duration-300 group"
                  >
